...
//###########################KORUS ROS#################################
std_msgs::Bool A1_flag_msg;
std_msgs::Bool A2_flag_msg;
ros::Publisher A1_flag_pub;
...
void detections_callback(const vision_msgs::Detection2DArray::ConstPtr &detections2dArray)
{
	...
	A1_flag_msg.data = A1_flag;
    A1_flag_pub.publish(A1_flag_msg);
    ...
    try
    {
		...
		for(int i = 0; i < detect_length; i++)
		{
			...
			if (Class_name == "A1"&& Area>7000)   //1500  A1인지 && Area>1150
            {
                A1_flag = true;
				cout << "Class_name [ " << detectionsCount << "] : " << Class_name << endl;
            }
            
            else if (Class_name == "A2" && Area>7000) //    A1이 인지된 이후    A2인지 && Area>1150
            {
                A2_flag = true;
				cout << "Class_name [ " << detectionsCount << "] : " << Class_name << endl;
                //cout<<"Area: "<<Area<<endl;
...
int main(int argc, char **argv)
{
    ...
    A1_flag_pub = nh.advertise<std_msgs::Bool>("Vision/A1_sign", 10);
	...
	while (ros::ok())
	{
		...
		pub.publish(msg);
		imshow("raw_image", frame1);

		if (callback == true)
		{
			//cout<<"Yolo V7 Running!!!"<< endl;
			Mat img;
		...
		{
			ROS_INFO("---- ");
    		ROS_INFO("Detect something");
			cout<<"A1_flag: "<<A1_flag<< " [1 = detected, 0 = not detected]" << endl;
            cout<<"A2_flag: "<<A2_flag<< " [1 = detected, 0 = not detected]" << endl;
			A1_flag_msg.data = A1_flag;
   			A1_flag_pub.publish(A1_flag_msg);
			A2_flag_msg.data = A2_flag;
   			A2_flag_pub.publish(A2_flag_msg);

            if (A1_flag == true || A2_flag == true)
            {
		...
		{	
			...
			cout<<"A1_flag: "<<A1_flag<< " [1 = detected, 0 = nothing detected]" << endl;
            cout<<"A2_flag: "<<A2_flag<< " [1 = detected, 0 = nothing detected]" << endl;
