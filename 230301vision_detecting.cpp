...
void detections_callback(const vision_msgs::Detection2DArray::ConstPtr &detections2dArray)
{
    ...
    try
    {
		...
		for(int i = 0; i < detect_length; i++)
		{
			...
			else if (Class_name == "A1" && Area>1150 && mission_flag==7)   //1500 A1일때 정지신호보냄 5초정지
            {
                count_A1 += 1;
                A_flag=true;
                cout<<"Area: "<<Area<<endl;
            ...
            else if (Class_name == "A2")    //   두번째 표지판 구간에들어가면 감속
            {
                ...
                cout << "Class_name.subscrib [ " << detectionsCount << "] : " << Class_name << " Area: " << Area << endl;
                ...
                if ((Area>2300) &&( (boundingBox2d.size_y)/(boundingBox2d.size_x)<=1.5) && (mission_flag==8))   //2700
                {
                    ...
                    if (Class_name == "A2" && mission_flag==8)    //A2일때 5km감속 주행
                    {
                        mission_B = 2;
                    }
			...
			else if ((Class_name == "A3") && (Area >2300) && ( (boundingBox2d.size_y)/(boundingBox2d.size_x)<=1.5) && (mission_flag==8))
			{
				...
				cout << "Class_name [ " << detectionsCount << "] : " << Class_name << " Area: " << Area << endl;
				speed_down = true;"/dev/video" + to_string(CAM_NUM);"/dev/video" + to_string(CAM_NUM);
				mission_B = 3;
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		if (detect == true)
		{
			...
			else if (count_A2 < count_A1 && count_A3 < count_A1)  //A1이라고 판단되었을때 mission_A = 1
			{
				mission_A = 1;
			}
			...
			Speed_pub.publish(mission_speed_down);  //감속 퍼블리시
